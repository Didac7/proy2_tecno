...
const cargarEstadoGPS = async () => {
  try {
    const response = await axios.get(`${page.props.appUrl}/api/gps/estado`);
    if (response.data.success) {
      gpsEstado.value = response.data;
      
      // Si el GPS está activo, iniciar envío automático
...
const iniciarGPS = async () => {
  ...
  try {
    ...
    const response = await axios.post(`${page.props.appUrl}/api/gps/iniciar`);
    ...
    if (response.data.success) {
      ...
      // Iniciar envío automático cada 10 segundos
      iniciarEnvioAutomatico();
    }
  } catch (error) {
    if (error.code) {
...
const finalizarGPS = async () => {
  ...
  try {
    const response = await axios.post(`${page.props.appUrl}/api/gps/finalizar`);
...
const enviarUbicacion = async (latitud, longitud) => {
  try {
    await axios.post(`${page.props.appUrl}/api/gps/actualizar`, {
      latitud,
      longitud,
    });
